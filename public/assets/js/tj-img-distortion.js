...
(function ($) {
	...
	const backgrounds = document.querySelectorAll(".tj-image-distortion");
	// If the element doesn't exist, exit the function
	if (!backgrounds?.length) return;

	backgrounds?.forEach(background => {
		// Read image source from data attribute
		const imageSrc = background.getAttribute("data-bg-image");
		...
		img.src = imageSrc; // <-- Here we use the data-bg-image attribute
